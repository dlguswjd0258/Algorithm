	...
	public static void main(String[] args) throws Exception {
		...
		while (T-- > 0) {
			...
			for (int i = 0; i < N; i++) {
				...
				emp[doc] = Integer.parseInt(st.nextToken()); // 서류 순위 위치에 면접 순위 저장 => 서류는 성적순으로 정렬됨.
			}

			// 2. 서류 1위의 면접 성적과 비교 했을 때 큰 값이 있으면 N--, 작은 값이 있으면 작은값으로 비교값 변경
			int comNum = emp[1];
